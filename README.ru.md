...
успешного продления перезагружает Nginx. Отдельный systemd-таймер ежедневно
перевыпускает CRL mTLS и перезагружает Nginx только после успешной проверки.
Не закрывайте порт 80 и не удаляйте DNS-запись домена.
...
чтобы добавить или обновить Nginx-конфиг и страницу-заглушку. Во время этого
обновления работающий HTTPS не переключается на HTTP; Nginx перезагружается
только после успешной проверки конфигурации.
...
Скрипт применяет патч `dns.uc`, сохраняет резервную копию, привязанную к
версии Forkop, и перезагружает Forkop. Дополнительные пакеты OpenWrt не
устанавливаются.
...
После отзыва скрипт обновляет CRL и перезагружает Nginx. Ежедневный таймер
поддерживает CRL действительным, даже если отзывов не было. Отозванный роутер
сразу теряет доступ к DoH.

## Продление сертификата роутера

Клиентский сертификат роутера действует **825 дней**. Он выдан приватным
центром сертификации mTLS, а не Let’s Encrypt, поэтому автоматически не
продлевается. Публичный сертификат Nginx продлевает Certbot; это отдельный
процесс и к сертификатам роутеров не относится.

Чтобы заменить сертификат без простоя DNS:

1. Выпустите новый сертификат с новым именем, например `openwrt-home-2028`.
2. Скопируйте новые `client.crt` и `client.key` на роутер поверх старых,
   сохранив права `644` и `600` соответственно.
3. Перезагрузите Forkop и проверьте работу DoH.
4. Отзовите старый сертификат через пункт `3` меню серверного скрипта.
...
непропатченный `dns.uc` только при совпадении сохранённой и установленной
версий Forkop, удаляет mTLS-настройки и файлы патча. Клиентские сертификат и
ключ остаются на месте: их пути задаёт пользователь, поэтому скрипт не может
безопасно считать их своими. Сам пакет Forkop остаётся установленным.
