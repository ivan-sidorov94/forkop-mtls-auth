...
непропатченный `dns.uc`, удаляет mTLS-настройки и файлы патча. Клиентские
сертификат и ключ остаются на месте: их пути задаёт пользователь, поэтому
скрипт не может безопасно считать их своими. Сам пакет Forkop остаётся
установленным.
